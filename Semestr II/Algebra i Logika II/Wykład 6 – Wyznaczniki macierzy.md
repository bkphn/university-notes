## Wyznacznik macierzy
Wyznacznik macierzy istnieje tylko w przypadku macierzy kwadratowych i informuje o tym, czy dana macierz jest odwracalna. Wyznacznik macierzy $\mathbf{A}$ zapisujemy jako $\det(\mathbf{A})$ lub $|\mathbf{A}|$. Używanie pionowych kresek zamiast $\det$ pozwala na usunięcie nawiasu: $$ \det\left(\begin{bmatrix} a_{11} & \dots & a_{1n} \\ \vdots & \ddots & \vdots \\ a_{m1} & \dots & a_{mn} \end{bmatrix}\right) = \begin{vmatrix} a_{11} & \dots & a_{1n} \\ \vdots & \ddots & \vdots \\ a_{m1} & \dots & a_{mn} \end{vmatrix} $$
