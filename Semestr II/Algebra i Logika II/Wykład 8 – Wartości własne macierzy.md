## Ślad macierzy
Ślad macierzy występuje tylko w przypadku macierzy kwadratowych. Śladem macierzy nazywamy sumę wszystkich elementów na jej głównej przekątnej i zapisujemy jako $\operatorname{tr} \mathbf{A}$: $$ \operatorname{tr} \mathbf{A}_{n \times n} = \sum_{i=1}^{n} a_{ii} = a_{11} + a_{22} + \dots + a_{nn} $$
**Własności śladu macierzy:** $$\operatorname{tr} \mathbf{A} + \operatorname{tr} \mathbf{B} = \operatorname{tr}(\mathbf{A} + \mathbf{B})$$ $$\operatorname{tr}(c\mathbf{A}) = c \operatorname{tr} \mathbf{A}$$
