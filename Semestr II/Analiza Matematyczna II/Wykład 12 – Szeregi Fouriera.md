...
$$ \langle f, g \rangle = \int_{-\pi}^{\pi} f(x)g(x) \, dx $$Mówimy, że funkcje $f(x)$ i $g(x)$ są **ortogonalne**, kiedy pole pod wykresem ich iloczynu wynosi zero, czyli $\langle f, g \rangle = 0$. Oznacza to, że funkcje te są „prostopadłe” do siebie w przestrzeni funkcji. Układ funkcji $(\sin 0x, \sin 1x, \sin 2x, \dots, \cos 0x, \cos 1x, \cos 2x, \dots)$ jest układem ortogonalnym. Zachodzi również: $$ \int_{-\pi}^{\pi} \sin mx \cdot \cos nx \, dx = 0, \quad m \neq n $$
## Szereg Fouriera
...
> Ciąg średnich arytmetycznych sum funkcji, zbiega **jednostajnie** $\rightrightarrows$ do funkcji $f$, czyli równomiernie i tak samo szybko dla każdego $x$: $$\frac{F_1(x) + F_2(x) + \dots + F_n(x)}{n} \rightrightarrows f(x) $$
