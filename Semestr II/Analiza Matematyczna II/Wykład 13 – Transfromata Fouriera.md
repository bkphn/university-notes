...
Ze wzorów Eulera możemy wyprowadzić następujące wzory na sinus i cosinus:

$$\sin nx = \frac{e^{inx} - e^{-inx}}{2i}, \quad \cos nx = \frac{e^{inx} + e^{-inx}}{2}$$

Dzięki tym wzorom możemy wprowadzić nowy sposób zapisu klasycznego szeregu Fouriera w postaci zespolonej:

$$\frac{a_0}{2} + \sum_{n=1}^{\infty} [a_n \cos nx + b_n \sin nx] = \sum_{n=-\infty}^{\infty} c_n e^{inx}$$
