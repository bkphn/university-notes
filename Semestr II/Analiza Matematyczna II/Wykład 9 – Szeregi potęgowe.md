...
> Szereg jest zbieżny.
## Koło zbieżności
Dla dowolnego $a_n \in \mathbb{C}$, $n \in \mathbb{N}$ szereg $\sum_{n=1}^{\infty} a_n z^n$ jest zbieżny bezwzględnie w kole $\{z : |z| < R\}$, gdzie: $$ R = \left(\lim_{n \to \infty} \sqrt[n]{|a_n|}\right)^{-1} $$
