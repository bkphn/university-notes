...
**Kodowaniem** nazywamy funkcję $c: \mathcal{A} \rightarrow \mathcal{B}^*$ przyporządkowującą litery alfabetu $\mathcal{A}$ słowom z alfabetu $\mathcal{B}$. Słowo $c(a)$ nazywamy słowem (wektorem) kodowym.

**Funkcję rozszerzenia** $\overline{c}: \mathcal{A}^* \rightarrow \mathcal{B}^*$ definiujemy jako:
...
* **Dekodowanie:** Funkcja odwrotna $\overline{c}^{-1}: \mathcal{B}^* \rightarrow \mathcal{A}^*$.
* Kodowanie jest jednoznacznie dekodowalne, jeżeli funkcja $c$ jest iniekcją.
## Kody
Zbiór $C = \operatorname{Im}_c = \{c(a_1), c(a_2), \dots, c(a_n)\} \subseteq \mathcal{B}^*$ nazywamy kodem, a wartość $l_{i}=l(c_{i})$ nazywamy długością słowa kodowego. Średnią długość słowa kodowego oznaczamy przez $l(C)$ i definiujemy następująco: $$ l(C) = \sum_{i=1}^n p_i \cdot l_i $$
### Kody powtórzeniowe
