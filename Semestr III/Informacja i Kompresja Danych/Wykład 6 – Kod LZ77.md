...
Dane są dwa słowa $\vec{a} = a_1a_2\dots a_h$, $\vec{b} = b_1b_2\dots b_h \in \mathcal{A}^h$. Mówimy, że $\vec{a} < \vec{b}$ (w porządku leksykograficznym), jeżeli istnieje taki indeks $k \in \{1, \dots, h\}$, że:> $$ \begin{cases} a_j = b_j, & j < k \\ a_j < b_j, & j = k \end{cases} $$
## Słownik i kody słownikowe
Słownik możemy zdefiniować jako niepusty, skończony zbiór słów (wektorów): $S = (\vec{m}_1, \vec{m}_2, \dots, \vec{m}_n)$, gdzie dla każdego $i$ zachodzi $\vec{m}_i \in \mathcal{A}^*$. Słowa te posiadają swoje indeksy $i$. Słowniki mogą być statyczne lub dynamiczne (zmieniające się w procesie kodowania).

Koder rozpoczyna pracę ze słownikiem początkowym $S_0 = (d_1, d_2, \dots, d_n)$, gdzie $\forall_i d_i = a_i$, przy ustalonym porządku liter $a_1 < a_2 < \dots < a_n$. Komunikat $\vec{m} = m_1m_2\dots m_l \in \mathcal{A}^*$ jest kodowany w słowo kodowe $c(\vec{m})$.
## Kod LZ77
