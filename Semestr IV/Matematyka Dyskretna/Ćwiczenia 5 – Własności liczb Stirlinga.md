...
$$x^{\overline{m+n}}=x^\overline{m}⋅(x+m)^\overline{n}$$ $$x^\underline{m+n}=x^\underline{m}⋅(x−m)^\underline{n}$$
## Własności liczb Stirlinga
Dla dowolnego $n\in\mathbb{N}_{+}$ zachodzą wzory:
- Dla $k=n$
$$\left[\begin{matrix} n \\ n \end{matrix} \right]=\left\{\begin{matrix} n \\ n \end{matrix} \right\}=\binom{n}{n}=1$$ Wybierając $n$ cykli z $n$-elementowego zbioru mamy tylko jedną możliwość utworzenia takowych: $[1], [2],\dots,[n−1],[n]$. Analogicznie możemy ułożyć n rozłącznych podzbiorów tylko na jeden sposób: $\{1\},\{2\},\dots,\{n−1\},\{n\}$.
