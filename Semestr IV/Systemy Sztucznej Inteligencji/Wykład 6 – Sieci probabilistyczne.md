...
Porównanie wektora do klasy abstrakcji na neuronach opisuje się najczęściej funkcją Gaussa. Dla jednego wymiaru wygląda ona następująco: $$g_{n}(x)=\frac{1}{\sigma\sqrt{2\pi}}\cdot e^{-\frac{(x-c)^2}{2\sigma^2}}$$
W praktyce porównujemy wielowymiarowy wektor wejściowy $\mathbf{x}$ z każdym zapisanym elementem $\mathbf{x}_n$ dla klasy abstrakcji $C_p$ za pomocą odległości: $$g_{n}^p(\mathbf{x})= \frac{1}{\sigma\sqrt{2\pi}}\cdot e^{-\frac{{\parallel \mathbf{x}-\mathbf{x}_{n}\parallel}^2}{2\sigma^2}}$$
Ostateczna wartość funkcji sumy na warstwie sumacyjnej dla badanej klasy abstrakcji $C_p$  wynosi: $$G^p(\mathbf{x})=\frac{1}{N\sigma\sqrt{2\pi}}\cdot\sum_{n=1}^N e^{-\frac{{\parallel \mathbf{x}-\mathbf{x}_{n} \parallel}^2}{2\sigma^2}}$$
## Uczenie sieci PNN
