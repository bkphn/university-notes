...
- **Testy równościowe** $$t(\mathbf{x})=\begin{cases}
1, \quad \operatorname{atr}(\mathbf{x})=u \\
0, \quad \operatorname{atr}(\mathbf{x})\neq u
\end{cases}$$
- **Testy przynależnościowe** $$t(\mathbf{x})=\begin{cases}
1, \quad \operatorname{atr}(\mathbf{x})\in U \\
0, \quad \operatorname{atr}(\mathbf{x})\not \in U
\end{cases}$$
- **Testy progowe** $$t(\mathbf{x})=\begin{cases}
...
Entropię warunkową $H(P \mid R)$ obliczamy ze wzoru: $$H(P\mid R)=\sum_{r\in R} \frac{|P^d|}{P}\cdot H(P_{r})=\sum_{r\in R} \frac{|P_{r}|}{|P|}\cdot \left( \sum \frac{|P_{r}^d|}{|P_{r}|} \cdot \log_{2}\left( \frac{|P_{r}^d|}{|P_{r}|} \right) \right)$$
...
Niech $Y=\{y_1,y_2,\dots, y_n \}$ będzie zbiorem obserwacji ułożonych rosnąco. **Współczynnik Giniego** zbioru $Y$ oznaczamy jako $G(Y)$ i definiujemy wzorem: $$G(Y)= \frac{\sum_{i=1}^n (2i-n-1)y_{i}}{n^2 \sum_{i=1}^n\left( \frac{y_{i}}{n} \right)}$$
